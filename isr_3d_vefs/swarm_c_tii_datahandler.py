...
def collect_data(swarm_c_tii_fn, start_time, end_time, time_step, iweight):
    # Load CDF file
    v = cdflib.CDF(swarm_c_tii_fn)
    ...
    while current_time <= end_time:
        ...
        print("swarm c tii los shape: ", los.shape)
        print("swarm c tii vlos shape: ", vlos.shape)
        print("swarm c tii coords shape: ", coords.shape)

        swarm_c_tii_data.append(lompe.Data(vlos, coordinates=coords, LOS=los, datatype='convection', scale=None, iweight=iweight))

        current_time = next_time
        
    return swarm_c_tii_data
