...
# -*- coding: utf-8 -*-
"""
Created on Fri Mar 29 23:11:37 2024
...
prime = "SW_OPER_MAGA_LR_1B"

DT = dt.timedelta(seconds = 5 * 60)


#t0 = dt.datetime(2017, 6, 16, 14, 00)
start_time = dt.datetime(2023, 3, 19, 6, 35)
end_time = dt.datetime(2023, 3, 19, 8, 35)
...
while current_time <= end_time:
    ...
    data = lompe.Data(values = dB * 1e-9, coordinates = coords, datatype = 'space_mag_full')

    model.add_data(data)
    model.run_inversion(l1 = 1, l2 = 0.1)
    lompe.lompeplot(model, include_data = True, apex = a, time = t0)
    ...
    def collect_data(dB, coords):
        swarm_a_mag_data = lompe.Data(dB, coords, datatype='space_mag_fac', error=30e-9, iweight=0.3)
        
        return swarm_a_mag_data
    
    

"""
fig, ax = plt.subplots()
xi, eta = p.geo2cube(np.rad2deg(lo), np.rad2deg(la))
ax.plot(xi, eta)
