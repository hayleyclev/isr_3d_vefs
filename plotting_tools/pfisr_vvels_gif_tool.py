...
start_time = datetime.datetime(2017, 6, 16, 14, 1, 0)
end_time = datetime.datetime(2017, 6, 16, 14, 59, 0)
minute_interval=datetime.timedelta(minutes=1)

for minute in range(1,60):
    current_time=start_time+datetime.timedelta(minutes=minute)
    # Use the same set of subplots axarr for both initial plot and plots inside the loop
    fig, axarr = plt.subplots(2, 2, sharex=True, sharey=True)
    ...
    vertical_line_time = start_time
    axarr[0, 0].axvline(x=mdates.date2num(vertical_line_time), color='magenta', linestyle='-')
    axarr[0, 1].axvline(x=mdates.date2num(vertical_line_time), color='yellow', linestyle='-')
    axarr[1, 0].axvline(x=mdates.date2num(vertical_line_time), color='magenta', linestyle='-')
    axarr[1, 1].axvline(x=mdates.date2num(vertical_line_time), color='magenta', linestyle='-')
